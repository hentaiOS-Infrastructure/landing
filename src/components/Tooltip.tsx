import clsx from "clsx";
import type { ComponentPropsWithoutRef, SVGProps } from "react";

interface TooltipProps extends ComponentPropsWithoutRef<"span"> {
  content: string;
}

const Tooltip = ({ children, className, ...props }: TooltipProps) => {
  return (
    <span className={clsx("text-black", className)} {...props}>
      {children}
      <MaterialSymbolsInfo className="relative ml-1 inline break-before-avoid break-after-avoid" />
    </span>
  );
};

function MaterialSymbolsInfo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 24 24" {...props}>
      <path
        fill="currentColor"
        d="M11 17h2v-6h-2Zm1-8q.425 0 .713-.288Q13 8.425 13 8t-.287-.713Q12.425 7 12 7t-.712.287Q11 7.575 11 8t.288.712Q11.575 9 12 9Zm0 13q-2.075 0-3.9-.788q-1.825-.787-3.175-2.137q-1.35-1.35-2.137-3.175Q2 14.075 2 12t.788-3.9q.787-1.825 2.137-3.175q1.35-1.35 3.175-2.138Q9.925 2 12 2t3.9.787q1.825.788 3.175 2.138q1.35 1.35 2.137 3.175Q22 9.925 22 12t-.788 3.9q-.787 1.825-2.137 3.175q-1.35 1.35-3.175 2.137Q14.075 22 12 22Z"
      ></path>
    </svg>
  );
}

export default Tooltip;
